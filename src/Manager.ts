...
/*
* Declare replaceAll's function on all strings.
 */
...
export class Manager {
    ...
    public cache: {
        ...
        giveaways: Array<{
            ...
            blockedUsers: Array<User["id"]>;
            blockedRoles: Array<Role["id"]>;
            participations: Array<User["id"]>;
            winners: Array<User["id"]>;
        }>;
    };
     ...
     */
    constructor(client: Client, {storage, updateInterval}: { storage: string; updateInterval: number; }) {
        this.client = client;
        // @ts-ignore
        this.cache = {};
        // @ts-ignore
        if (storage) this.cache.storage = storage;
        else new Error("There's no valid storage file in JSON (.json).");
        if (updateInterval) this.cache.updateInterval = updateInterval
        else this.cache.updateInterval = 30000;
        this.cache.interval = [];
        if (!existsSync(this.cache.storage) || !String(this.cache.storage).endsWith(".json")) throw new Error("There's no valid storage file in JSON (.json).");
        this.cache.giveaways = require(this.cache.storage);
        this.client.on('interactionCreate', (interaction: Interaction) => {
            // @ts-ignore
            const id = interaction.customId || null;
            if (!id || interaction.user.bot) return;
            let giveaway = this.cache.giveaways.find(x => x.id === id && x.ended === false);
            if (interaction.isButton() && giveaway) {
                // @ts-ignore
                if (interaction.member.roles.cache.find((r) => giveaway.blockedRoles.includes(r.id)) || giveaway.blockedUsers.includes(interaction.user.id)) return;
    ...
    */
    create(message: Message, data: {
        blockedRoles?: Array<Role>;
        blockedUsers?: Array<Role>;
        time: number;
        prize: string;
        winnerCount: number;
        channel?: any;
    ...
    */
    ...
    private end(giveaway: {
        messageGiveaway?: string | undefined; id?: any; messageID?: string | undefined; channelID?: string | undefined; guildID?: string | undefined; authorID?: any; startAt?: any; endAt?: any; ended: any; prize?: any; winnerCount: any; data?: { content: { text: string; participate: string; removeParticipate: string; winMessage: string; noWinner: string; }; embed: { color: number; author: boolean; title: string; footer: { start: string; end: string; text: string; icon_url: string; }; description: { start: string; end: string; }; timestamp: number; }; button: "ACTION_ROW" | "BUTTON" | "SELECT_MENU"; } | undefined; participations: any; winners: any; blockedUsers: any; blockedRoles: any;
        ...
        if (rerolled) this.send(giveaway, winners, rerolled);
        else this.send(giveaway, winners);
        giveaway.winners = winners;
        giveaway.ended = true;
        this.saveStorage();
    }

    private async send(json: any, winners?: any[], rerolled?: boolean | undefined) {
        const giveaway = this.cache.giveaways.find(x => x.id === json.id);
        if (!giveaway) return;
        let data: { content: { text: string, participate: string, removeParticipate: string, winMessage: string, noWinner: string }, embed: { color: number, author: boolean, title: string, footer: { text: string, icon_url: string }, description: string, timestamp: number }, button: MessageComponentType } = JSON.parse(JSON.stringify(giveaway.data).replaceAll("{prize}", json.prize).replaceAll("{user}", `<@${json.authorID}>`).replaceAll("{duration}", `<t:${json.endAt}:R>`)),
            // @ts-ignore
            channel: TextChannel = await this.client.channels.cache.get(giveaway.channelID);
        if (!channel) {
            giveaway.ended = true;
        ...
        if (winners) {
            ...
            if (winners.length) {
                const winner = winners.map((x: any) => `<@${x}>`).join(", ");
            ...
            giveaway.ended = true;
            if (rerolled) this.client.emit("giveawayRerolled", giveaway);
            else this.client.emit("giveawayEnded", giveaway);
            if (data.embed.footer) data.embed.footer = {
                text: giveaway.data.embed.footer.end,
                icon_url: data.embed.footer.icon_url
            };
        } else {
            if (data.embed.description) data.embed.description = giveaway.data.embed.description.start;
            if (data.embed.footer) data.embed.footer = {
...
function progressBar(value: number, maxValue: number) {
